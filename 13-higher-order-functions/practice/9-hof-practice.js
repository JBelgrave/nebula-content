...
function greaterHundred(arr) {
    arr.sort((a, b) => a - b)
    return arr.find(number => number > 100)
}
console.log(greaterHundred(nums))
...
function sortAge(arr) {
    return arr.sort((a, b) => a.birthYear - b.birthYear)
}
...
function filterYear(arr) {
    return arr.filter(item => item.make < 2000)
}
...
function protect(arr){
    return arr.map((pNum) => {
        return `${pNum[0]}##-###-#${pNum[9]}${pNum[10]}${pNum[11]}`
    })
}
console.log(protect(phoneNums))
...
function filterNameLength(arr) {
    return arr.filter(name => name.length > 5)
}
console.log(filterNameLength(peopleArray))
...
function filterSortAge(arr) {
    const filtered = arr.filter(person => person.age > 20 && person.age < 35)
    return filtered.sort((a, b) => a.age - b.age)
}
console.log(filterSortAge(people1))
...
function sortExpiration(arr){
    return arr.sort((a, b) => a.exp - b.exp)[0]
}
console.log(sortExpiration(food))