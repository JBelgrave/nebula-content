...
SELECT plane.name as plane_name, model.engines as number FROM plane JOIN model ON plane.modelcod = model.modelcod;

-- 4. How many flights with a 2 or 3 hour duration are there in the database? (number)
SELECT Count(*) FROM flight WHERE duration BETWEEN 2 and 3;
...
SELECT country, COUNT(*) as num FROM airport GROUP BY country;
...
SELECT airportcod FROM airport WHERE city IN ('Porto', 'Lisboa')
SELECT flightcod FROM flight WHERE fromairportcod = 1 AND toairportcod = 3;

-- 11. How many airports are there in each country? (country, number); show only countries with more than 2 airports.
SELECT country, COUNT(*) as num FROM airport GROUP BY country HAVING COUNT(*) > 2;
...
SELECT model.modelcod, count(model.modelcod) FROM plane
JOIN model ON plane.modelcod = model.modelcod GROUP BY model.modelcod ORDER BY count(model.modelcod) DESC;